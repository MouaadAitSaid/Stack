...
export class ConfigService {
  ...
  public load(): Promise<any> {
    if(this.loaded) {
      ...
      const configurationObservable = this.http.get(`./assets/config/config.json`); // path is relative to that for app's index.html
      configurationObservable
        .catch(error => {
          alert(`error loading configuration: ${JSON.stringify(error)}`);
